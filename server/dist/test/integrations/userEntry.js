...
describe("Registering a new user", function () {
    ...
    describe("Save a user", function () {
        it("should save a user", function (done) {
            chai.request(baseUrl).post('/register').send(input).end(function (err, res) {
    ...
    describe('Authenticate user', function () {
        it('should verify user and redirect to login page', function (done) {
            chai.request(baseUrl).get(`/verify/${token}`).end(function (err, res) {
                expect(loginUrl);
                done();
            });
        });
    ...
    describe('Login user', function () {
        it('should login user', function (done) {
            chai.request(baseUrl).post('/login').send(input).end(function (err, res) {
                expect(res.status).to.equal(200);
                done();
            });
        });
